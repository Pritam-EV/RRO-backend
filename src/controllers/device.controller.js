...
const EnergyLog = require("../models/EnergyLog.model");
...
exports.getOverview = asyncHandler(async (req, res) => {
  ...
  // Latest reading
  const latest = await EnergyLog.findOne({ deviceId: device._id }).sort({ recordedAt: -1 });
...
exports.getUsage = asyncHandler(async (req, res) => {
  ...
  const logs = await EnergyLog.find(filter)
    .sort({ recordedAt: -1 })
...
exports.postEnergyLog = asyncHandler(async (req, res) => {
